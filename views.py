...
def add_category(request):
    if request.method == 'POST':
        n = request.POST['name']
        u = request.POST['unit']
        Category.objects.create(name=n, unit=u)
...
def donate_blood(request):
    if request.method == "POST":
        ...
        cat = Category.objects.get(id=bg)
        user = UserProfile.objects.get(user=request.user)
        data = Blood_Donation.objects.create(blood_group=cat, user=user, purpose="Blood Donor", status="Pending", place=place, desease=desease)
